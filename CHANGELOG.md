...
**修一个只在"两个注音插件同时开着"时才暴露的洞：同步修复钩子被顶掉了。**

`window.__ktRepairLine` 是全局的，装了共存补丁的 jp-furigana 重建完一行就调它。
2.1.2 之前本插件是**直接赋值**挂上去的 —— 只要它加载得比「拉丁字母片假名注音」晚，
就会把对方已经挂好的钩子顶掉，那边从此收不到同步通知，只能靠 MutationObserver
等下一帧补，中间那一帧就是可见的一闪（正是 2.0.14 修过的那种病，只是换了成因）。
谁先加载不该决定谁的功能还在。
